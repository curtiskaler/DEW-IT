...
public abstract class EventBase
{
    ...
    /// <returns>The <see cref="SubscriptionToken"/> that uniquely identifies the subcscriber.</returns>
    ...
    /// <summary>
    /// Calls all the execution strategies exposed by the list of <see cref="IEventSubscription"/>s.
    /// </summary>
    ...
    /// the subscribers from the list that return a <see langword="null"/>
    /// <see cref="Action{T}"/> when calling the
    ...
    /// </summary>
    /// <param name="token">The <see cref="SubscriptionToken"/> returned
    /// by <see cref="EventBase"/> while subscribing to the event.</param>
    public virtual void Unsubscribe(SubscriptionToken token)
    {
        lock (Subscriptions)
        {
            IEventSubscription subscription = Subscriptions.FirstOrDefault(evt =>
            evt.SubscriptionToken == token);
    ...
    /// </summary>
    /// <param name="token"> The <see cref="SubscriptionToken"/> returned
    /// by <see cref="EventBase"/> when subscribing to the event.
    /// </param>
    /// <returns> <see langword="true"/> if there is a <see cref="SubscriptionToken"/>
    /// that matches; otherwise, <see langword="false"/>.
    /// </returns>
    ...
    /// <summary>
    /// Force the PubSubEvent to remove any subscriptions that no longer
